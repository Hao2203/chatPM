...
pub fn rrf_fuse(ranked_a: &[SearchResult], ranked_b: &[SearchResult], k: f64) -> Vec<SearchResult> {
    ...
    let mut score_map: HashMap<ChunkId, (f64, &SearchResult)> = HashMap::new();
...
mod tests {
    ...
    /// 创建用于测试的确定性 ChunkId。
    fn cid(n: u8) -> ChunkId {
        ChunkId::from_uuid(uuid::Uuid::from_bytes([n; 16]))
    }

    fn make_result(chunk_id: ChunkId, score: f32) -> SearchResult {
        SearchResult {
            chunk_id,
    ...
    fn rrf_fuse_empty_b_returns_a() {
        let a = vec![
            make_result(cid(1), 0.9),
            make_result(cid(2), 0.8),
            make_result(cid(3), 0.7),
        ];
        let result = rrf_fuse(&a, &[], 60.0);
        assert_eq!(result.len(), 3);
        // 第一个元素排名最高（分数最高）
        assert_eq!(result[0].chunk_id, cid(1));
    }

    #[test]
    fn rrf_fuse_empty_a_returns_b() {
        let b = vec![
            make_result(cid(24), 0.9),
            make_result(cid(25), 0.8),
        ];
        let result = rrf_fuse(&[], &b, 60.0);
        assert_eq!(result.len(), 2);
        assert_eq!(result[0].chunk_id, cid(24));
    }
    ...
    fn rrf_fuse_deduplicates_by_chunk_id() {
        ...
        assert!(
            (result[0].score - expected as f32).abs() < 0.001,
            "expected {}, got {}",
            expected,
            result[0].score
        );
    ...
    fn rrf_fuse_mixed_overlap() {
        ...
        let only_a = cid(1);
        let only_b = cid(2);
        let a = vec![make_result(only_a, 0.9), make_result(shared, 0.8)];
        let b = vec![make_result(shared, 0.6), make_result(only_b, 0.5)];
        let result = rrf_fuse(&a, &b, 60.0);
        // 应该有三个唯一结果
        assert_eq!(result.len(), 3);
        // "shared" 因双重排名分数最高
    ...
    fn rrf_fuse_same_list_equals_original_order() {
        let a = vec![
            make_result(cid(120), 0.9),
            make_result(cid(121), 0.8),
            make_result(cid(122), 0.7),
        ];
        // 两个相同列表融合 → 每个条目 RRF 分数翻倍，但相对顺序不变
        let result = rrf_fuse(&a, &a, 60.0);
        assert_eq!(result.len(), 3);
        assert_eq!(result[0].chunk_id, cid(120));
        assert_eq!(result[1].chunk_id, cid(121));
        assert_eq!(result[2].chunk_id, cid(122));
    }
    ...
    fn rrf_k_parameter_affects_scores() {
        ...
        assert!(
            ...
            with_small_k[0].score,
            with_large_k[0].score
        );
    }
}
